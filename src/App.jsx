  ...
  return (
    <figure
      className={cn(
        ...
        ` relative w-64 cursor-pointer overflow-hidden rounded-xl border p-4 ${isNew && 'min-w-[280px] w-auto'}`,
        // light styles
    ...
    >
      <div className={`flex flex-row items-center gap-2`}>
        ...
        {freeze && isNew ? <div className='text-4xl font-bold text-black dark:text-white py-2'> {organization_name || contact_name} </div> : '' }
          ...
          {/* {isEmpty && ( */}
            <>
              <p className="text-xs font-medium dark:text-white/40">สนับสนุน {donate_total} ต้น {/* <span className='text-yellow-600'>{Number(donate_total).toLocaleString()}</span> ต้น */}</p>
